...
it copies the built `target/kubeflow-backend-*.jar`, exposes `8080`, and runs
...
| File | Contents |
| --- | --- |
| `namespace.yml` | The `kubeflow` namespace. |
| `deployment.yml` | `kubeflow-backend` Deployment (profile `k8s`, `DB_*` from a Secret, `/actuator/health` probes). |
...
- `DB_USERNAME` / `DB_PASSWORD` come from the `kubeflow-postgres-secret` Secret.
- `KUBEFLOW_STALE_THRESHOLD_SECONDS` overrides the stale cleanup window.
...
- **Exporter:** `otlphttp/kubeflow_backend` with `encoding: json`, endpoint from
  the `KUBEFLOW_BACKEND_ENDPOINT` env var.
  - Backend on the host (local dev): `http://host.docker.internal:8080`.
  - Backend in-cluster: `http://kubeflow-backend.kubeflow.svc.cluster.local:8080`.
...
- `k8s.container.memory_limit_utilization` is intentionally **not** enabled on
  ...
  divided by `kubeflow.memory-limit-bytes`.
...
- If the backend runs locally (not in-cluster), point the collector's
  `KUBEFLOW_BACKEND_ENDPOINT` at the host and restart the collector.
